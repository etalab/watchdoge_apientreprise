...
def env_info
  puts "Running on #{Rails.env.to_s.green} env (#{Rails.application.config_for(:secrets)['apie_base_uri']}) with #{Rails.application.config.thread_number.to_s.yellow} threads"
end
